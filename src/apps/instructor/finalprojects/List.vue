<template>
    <div class="padded">
        <el-row :gutter="24">
            ...
            <el-col :span="8">
                <StudentForm v-if="currentSession" @refresh="handleRefresh" :session="currentSession" :labs="instructorLabs" @bookStudents="handleBooking" :sessions="sessions"></StudentForm>
            </el-col>
            <el-col :span="8">
        ...
        <br>
        <el-button @click="handleTestReload">
            <fa-icon name="refresh"></fa-icon> Refresh</el-button>
    </div>
</template>
<script>
           ...
           // console.log(opts.students)
            ...
            if (opts && opts.students) {
                Promise.all(
                    ...
                    vm.notify('Success', 'Student booking confirmed')
                    vm.reloadInstructor()
                }).catch((error) => {
                    const errorMsg = error.body.error.message || ''
